...
const stageTone = (stage: StageName) => stage === "Closed Lost" ? "lost" : pipelineGroupForStage(stage).key;
...
export function CrmApp() {
  ...
  const saveActivity = async (activity: ActivityDraft) => {
    const result = await mutate(
      ...
      activity.id ? "Đã cập nhật dòng Follow-up." : activity.nextStep ? "Đã lưu Follow-up và cập nhật Next Step hiện tại." : "Đã lưu cập nhật khách hàng.",
    );
...
function Pipeline({ opportunities, onOpen, onMove, saving }: { opportunities: Opportunity[]; onOpen: (item: Opportunity) => void; onMove: (item: Opportunity, direction: -1 | 1) => void; saving: boolean }) {
  return <>
    ...
    <div className="pipeline-board">{PIPELINE_GROUPS.map((column, columnIndex) => {
      ...
      return <section className={`pipeline-column pipeline-column-${column.key}`} key={column.key}>
        ...
        <div className="pipeline-cards">{cards.length === 0 ? <div className="empty-column">Chưa có deal</div> : cards.map((card) => <article className="pipeline-card" key={card.id}>
          <button className="card-main" onClick={() => onOpen(card)}><div className="card-top"><Temperature value={card.temperature}/><span className="card-score">{card.score}</span></div><strong>{card.companyName}</strong><span className={`pipeline-stage-detail stage-tone-${stageTone(card.stage)}`}>{stageDisplayLabel(card.stage)}</span><p>{card.productApplication}</p><div className="card-value">{money(card.estimatedValue)}</div><div className="card-detail"><Icon name="person" size={14}/><span>{card.contactName}</span></div><div className="card-detail"><Icon name="clock" size={14}/><span>{card.nextStepDue ? shortDate(card.nextStepDue) : card.status === "Open" ? "Thiếu Next Step" : "Đã đóng dự án"}</span></div></button>
...
function Records({ items, search, setSearch, stageFilter, setStageFilter, temperatureFilter, setTemperatureFilter, typeFilter, setTypeFilter, onOpen, onCreate, onQuote, onDelete, saving }: {
  ...
  return <>
    ...
    <section className="panel records-panel">
      <div className="table-scroll"><table><thead><tr><th className="stt-column">STT</th><th>Trading Partner / Khách hàng</th><th>Liên hệ</th><th>Cơ hội / End-User</th><th>Giai đoạn</th><th>Điểm</th><th>Giá trị</th><th>Next Step</th><th /></tr></thead><tbody>{items.map((item, index) => <tr key={item.id} onClick={() => onOpen(item)}><td className="stt-cell">{index + 1}</td><td><div className="company-cell"><div className="company-avatar">{item.companyName.slice(0, 1)}</div><div><strong>{item.companyName}</strong><span>{item.customerCode ? `Mã ${item.customerCode} · ` : ""}{item.accountType} · {item.region}</span></div></div></td><td><strong>{item.contactName}</strong><span>{item.title || item.department}</span></td><td><strong>{item.productApplication}</strong><span>{item.endUserCompany ? `End-User: ${item.endUserCompany}` : item.id}</span></td><td><span className={`stage-pill stage-tone-${stageTone(item.stage)}`}>{stageDisplayLabel(item.stage)}</span></td><td><Temperature value={item.temperature} score={item.score}/></td><td><strong>{compactMoney(item.estimatedValue)}</strong><span>kỳ vọng {compactMoney(item.weightedValue)}</span></td><td><strong>{item.nextStep || "Chưa có"}</strong><span>{shortDate(item.nextStepDue)}</span></td><td><div className="table-actions"><button className="table-quote" aria-label="Tạo báo giá" title="Tạo báo giá" onClick={(event) => { event.stopPropagation(); onQuote(item); }}><Icon name="file" size={16}/></button><button className="table-edit" aria-label="Sửa" title="Sửa"><Icon name="edit" size={16}/></button><button className="table-delete" aria-label="Xóa cơ hội" title="Xóa cơ hội" disabled={saving} onClick={(event) => { event.stopPropagation(); onDelete(item); }}><Icon name="trash" size={15}/></button></div></td></tr>)}</tbody></table></div>
      ...
      <form onSubmit={onSubmit} autoComplete="off">
        <div className="modal-body">
          ...
          <section className="form-section"><div className="section-title"><div><Icon name="briefcase" size={18}/></div><span><strong>Cơ hội bán hàng</strong><small>Opportunity & Next Step</small></span></div><div className="form-grid">
            ...
            <FormField label="Giai đoạn" required hint="Mỗi giai đoạn có một màu nhận diện"><select className={`crm-stage-select crm-stage-select-${selectedGroup.key}`} value={selectedGroup.key} onChange={(e) => changePipelineGroup(e.target.value)}>{PIPELINE_GROUPS.map((group) => <option value={group.key} key={group.key}>{group.label}</option>)}</select></FormField>
