  ...
  return <section className="form-section follow-up-section"><div className="section-title"><div>↻</div><span><strong>Nhật ký Follow-up & Next Step</strong><small>Mỗi lần chăm sóc là một dòng; chỉ dòng được tích mới sang báo cáo tuần</small></span><button type="button" className="secondary-button follow-up-add" onClick={add}>＋ Thêm dòng</button></div>
    {editorOpen && <div className="follow-up-editor">
      <div className="follow-up-editor-grid">
        ...
        <label className="follow-up-wide"><span>Thông tin Follow-up <b>*</b></span><textarea rows={3} required value={draft.summary} onChange={(event) => update("summary", event.target.value)} placeholder="Đã trao đổi gì, khách cung cấp thêm thông tin nào..."/></label>
        <label className="follow-up-wide"><span>Kết quả / phản hồi</span><textarea rows={2} value={draft.outcome} onChange={(event) => update("outcome", event.target.value)} placeholder="Kết quả sau lần liên hệ này"/></label>
        <label><span>Next Step mới</span><input value={draft.nextStep} onChange={(event) => update("nextStep", event.target.value)} placeholder="Hành động tiếp theo"/></label>
        <label><span>Hạn Next Step</span><input type="date" value={draft.dueDate} onChange={(event) => update("dueDate", event.target.value)}/></label>
      </div>
      ...
      <div className="follow-up-editor-actions"><button type="button" className="secondary-button" onClick={() => setEditorOpen(false)}>Hủy</button><button type="button" className="primary-button" disabled={saving || !draft.summary.trim()} onClick={() => void save()}>{saving ? "Đang lưu..." : draft.id ? "Lưu thay đổi" : "Lưu Follow-up"}</button></div>
    </div>}
    {rows.length === 0 ? <div className="follow-up-empty"><strong>Chưa có lịch sử chăm sóc</strong><span>Bấm “Thêm dòng” sau mỗi lần Email, gọi điện, Zalo hoặc gặp khách.</span></div> : <div className="follow-up-list"><div className="follow-up-head"><span>Ngày / Kênh</span><span>Thông tin & kết quả</span><span>Next Step</span><span>Báo cáo tuần</span><span /></div>{rows.map((activity) => <article key={activity.id} className={activity.includeInWeeklyReport ? "is-reported" : ""}>
      ...
      <div><strong>{activity.summary}</strong><span>{activity.outcome || "Chưa ghi kết quả"}</span></div>
