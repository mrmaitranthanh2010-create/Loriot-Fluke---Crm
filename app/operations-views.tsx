...
export function WeeklyReportsView({ opportunities, activities, onError, onNotice }: { opportunities: Opportunity[]; activities: Activity[]; onError: (message: string) => void; onNotice: (message: string) => void }) {
  ...
  const [planInheritedFrom, setPlanInheritedFrom] = useState("");
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const inputRef = useRef<HTMLInputElement>(null);
  ...
  const load = useCallback(async (weekStart: string) => {
    ...
    try {
      ...
      const result = await response.json() as { report?: WeeklyReport; recent?: WeeklyReportSummary[]; planInheritedFrom?: string; error?: string };
      if (!response.ok || !result.report) throw new Error(result.error || "Không thể tải báo cáo tuần.");
      setReport(result.report); setRecent(result.recent ?? []); setPlanInheritedFrom(result.planInheritedFrom ?? "");
  ...
  return <>
    ...
    <section className="weekly-flow" aria-label="Quy trình vận hành báo cáo tuần"><article><span>1 · THỨ HAI–CHỦ NHẬT</span><strong>Ghi nhận hành động & kết quả</strong><p>Cập nhật Follow-up trong CRM và chọn những thông tin đủ giá trị cho báo cáo tuần {report.weekNumber}.</p></article><i>→</i><article><span>2 · CHỦ NHẬT</span><strong>Chốt báo cáo tuần {report.weekNumber}</strong><p>Lấy cập nhật từ CRM, kiểm tra kết quả từ {shortDate(report.weekStart)} đến {shortDate(report.weekEnd)}.</p></article><i>→</i><article><span>3 · TUẦN KẾ TIẾP</span><strong>Lập kế hoạch tuần {planWeekNumber}</strong><p>Chỉnh kế hoạch từ {shortDate(planWeek.weekStart)} đến {shortDate(planWeek.weekEnd)} rồi xuất Excel.</p></article></section>
    <section className="panel weekly-section"><div className="panel-header"><div><span>WEEKLY PLANNER · TUẦN KẾ TIẾP</span><h2>Kế hoạch tuần {planWeekNumber} · {shortDate(planWeek.weekStart)} – {shortDate(planWeek.weekEnd)}</h2></div><small className="weekly-export-note">{planInheritedFrom ? `Đã giữ nội dung từ báo cáo W${companyWeekNumber(planInheritedFrom)} để anh chỉnh sửa` : "Xuất vào Weekly Planner của tuần kế tiếp"}</small></div><div className="weekly-plan-grid">{report.plan.map((item, index) => <article key={item.dayKey}><header><span className="weekly-day-number">{String(index + 1).padStart(2, "0")}</span><div><strong>{item.label}</strong><span>{shortDate(addDays(planWeek.weekStart, index))}</span></div></header><label><span>Main Activity · Hoạt động chính</span><textarea rows={4} value={item.mainActivity} onChange={(event) => updatePlan(index, "mainActivity", event.target.value)} placeholder="Các hoạt động chính trong ngày..."/></label><div><label><span>Location · Địa điểm</span><input value={item.location} onChange={(event) => updatePlan(index, "location", event.target.value)}/></label><label><span>Remarks · Kết quả dự kiến</span><input value={item.remarks} onChange={(event) => updatePlan(index, "remarks", event.target.value)}/></label></div></article>)}</div></section>
    ...
    <div className="weekly-savebar"><span>{locked ? "Bản hoàn thành được giữ nguyên cho đến khi bạn chủ động mở lại." : "Dữ liệu được lưu theo từng tuần; Chủ Nhật anh chọn “Hoàn thành” rồi xuất Excel."}</span><div>{locked ? <button className="secondary-button" disabled={saving} onClick={() => void save("Draft")}>Mở lại chỉnh sửa</button> : <><button className="secondary-button" disabled={saving} onClick={() => void save("Draft")}>Lưu nháp</button><button className="primary-button" disabled={saving} onClick={() => void save("Submitted")}>Hoàn thành báo cáo</button></>}</div></div>
  </>;
}
