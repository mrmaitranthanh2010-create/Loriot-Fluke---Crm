...
export function WeeklyReportsView({ opportunities, activities, onError, onNotice }: { opportunities: Opportunity[]; activities: Activity[]; onError: (message: string) => void; onNotice: (message: string) => void }) {
  ...
  return <>
    ...
    <section className="weekly-flow" aria-label="Quy trình vận hành báo cáo tuần"><article><span>1 · THỨ HAI</span><strong>Lập kế hoạch</strong><p>Chọn đúng tuần, ghi việc chính từng ngày và kết quả dự kiến cần đạt.</p></article><i>→</i><article><span>2 · HẰNG NGÀY</span><strong>Cập nhật Follow-up trong CRM</strong><p>Thêm từng lần liên hệ và chỉ tích những dòng đủ giá trị để đưa vào báo cáo tuần.</p></article><i>→</i><article><span>3 · THỨ SÁU</span><strong>Chốt kết quả & xuất báo cáo</strong><p>Bấm “Lấy cập nhật từ CRM”, kiểm tra kết quả, chọn Hoàn thành rồi xuất Excel.</p></article></section>
